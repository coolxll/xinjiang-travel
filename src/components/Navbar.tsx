    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs transition-all w-full max-w-full overflow-x-clip">
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-1.5 sm:gap-4">
          ...
          {/* Center: 3-Mode Primary Switcher Pill */}
          <div className="bg-slate-100 p-0.5 sm:p-1 rounded-2xl border border-slate-200/80 flex items-center gap-0.5 sm:gap-1 shadow-2xs flex-shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl text-xs font-bold transition-all ${
                pageMode === 'main'
            ...
            {/* 2. Standalone Roadbook Mode (Feature) */}
            <button
              onClick={() => onSwitchPageMode('roadbook')}
              className={`flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl text-xs font-black transition-all ${
                pageMode === 'roadbook'
                  ? 'bg-emerald-600 text-white shadow-xs'
                  : 'text-emerald-700 hover:bg-emerald-50'
              }`}
              title="进入行程中每日路书独立伴侣模式"
            >
              <BookOpen className="w-3.5 h-3.5" />
              <span>每日路书</span>
              <span className="hidden lg:inline-flex text-[9px] px-1 py-0.2 rounded font-extrabold bg-emerald-500/20 text-emerald-800">
                行程伴侣
              </span>
            </button>

            {/* 3. Alternative Plans Mode */}
            <button
            ...
            <nav className="hidden 2xl:flex items-center space-x-1 flex-shrink min-w-0">
              {mainNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-amber-600' : item.isSpecial ? 'text-emerald-600' : 'text-slate-400'}`} />
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            {pageMode !== 'main' ? (
              ...
              >
                ...
                <span className="hidden sm:inline">返回大纲</span>
              </button>
            ) : (
              <button
                onClick={onOpenPrint}
