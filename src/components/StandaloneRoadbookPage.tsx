...
export const StandaloneRoadbookPage: React.FC<StandaloneRoadbookPageProps> = ({
  ...
  const [selectedDayId, setSelectedDayId] = useState<string>(() => {
    return localStorage.getItem('xinjiang_roadbook_active_day') || 'day-2';
  });
  ...
  // Save selected active day
  const handleSelectDay = (dayId: string) => {
    setSelectedDayId(dayId);
    localStorage.setItem('xinjiang_roadbook_active_day', dayId);
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 py-6 sm:py-10">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">

        {/* 1. Page Header & Companion Mode HUD */}
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-white rounded-3xl p-5 sm:p-8 shadow-xl border border-slate-700/60">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white flex items-center gap-2">
                <span>北疆每日自驾路书与导航中心</span>
              </h1>
              <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-2xl leading-relaxed">
                专为在途自驾打造：实时里程进度跟踪、逐日高德地图与导航点位一键唤起、时间节奏、加油餐饮及避坑底线。
              </p>
            </div>
        ...
        {/* 2. Route Mileage & Realtime Progress Tracker (Feature 1) */}
        ...
        {/* 3. Horizontal Day Tabs Switcher */}
        <div className="bg-white rounded-2xl border border-slate-200 p-2 shadow-xs">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            {filteredDays.map((day) => {
              ...
              const isProgressDay = day.dayNumber === savedProgressDayNumber;
              const isCompleted = day.dayNumber < savedProgressDayNumber;
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-2 rounded-xl text-left border transition-all ${
                    isSelected
                      ...
                      : isProgressDay
                      ? 'bg-amber-50 text-slate-800 border-amber-300 ring-2 ring-amber-400/40'
                      : isCompleted
                      ? 'bg-emerald-50/50 text-slate-700 border-emerald-200 hover:bg-emerald-100/50'
                ...
                >
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    {isProgressDay && !isSelected && (
                      <span className="text-[9px] px-1 rounded font-bold bg-amber-500 text-white animate-pulse">
                        今天
                      </span>
                    )}
                  </div>
        ...
        {/* 4. Active Day Main Work Area: High-Priority Embedded Amap + Detail Card */}
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>高德导航到 {primaryDest.name.slice(0, 8)}</span>
              </a>
            </div>
          </div>

          {/* Feature 2: Embedded AutoNavi Map */}
          ...
          {/* 5. Detailed Daily Itinerary Card */}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
              ...
              {nextDay ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black shadow-xs transition-colors"
                >
